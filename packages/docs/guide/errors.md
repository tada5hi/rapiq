...
| `SCHEMA_VALIDATOR_ASYNC_REQUIRES_ASYNC_PARSER` | `parse()` (or a synchronous codec method) encountered an async validator (a filter validator, a key validation hook or a groups / aggregates hook); use the corresponding `Async` method |
...
- `BuildError` / `MergeError` / `AdapterError` / `SchemaError` on the server → **500**: these mean *your* code produced or forwarded something invalid. The exception is an `AdapterError` carrying `KEY_VALUE_INVALID`: an operand the column's type cannot hold, which is a **400** when it came from the client. The adapter cannot tell who wrote the operand, so a condition your own code injects (a validate-hook replacement, a policy conjunct) must hold valid values for its column, or it raises the same code.
