...
When a query carries [groups or aggregates](/guide/grouping#grouped-mode), sorts may name output keys only (`sort=-count`, `sort=createdAt`, `sort=sumAmount`); record columns and relation paths are rejected under the usual drop or throw policy. The schema's `default`, `validate` / `validateMany` and `indexed` do not apply. Every group key the sort does not name is appended ascending as a tie-breaker, so the order is total and `offset` paging is stable; without a sort, rows are ordered by every group key ascending. Where a `null` group lands differs by engine ([details](/guide/grouping#grouped-mode)).
