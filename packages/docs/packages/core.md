...
| **Schema system** | `defineSchema`, `Schema`, `SchemaRegistry` (incl. `getAll()`), `RegisteredSchema`, per-parameter `define*Schema` factories, `ResolutionScope`, `Schema.describe()` | [Schemas & Validation](/guide/schemas) |
