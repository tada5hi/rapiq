...
| Option | Description |
|---|---|
| `relations.joinAndSelect` | Hydrate **every** joined relation fully, including those joined only for a filter or sort. Off by default. This legacy blanket override also disables fieldset narrowing: under it, an `include`d relation is always a complete subtree. Without it, an `include`d relation hydrates as a complete subtree only while the query carries no direct fieldset for it: a per-relation fieldset (a client-sent `fields[child]=...` or the child schema's `fields.default`/`allowed`) narrows the include to exactly those columns plus the relation's primary key. A relation joined purely for filtering/sorting stays unselected (its columns are hydrated only when a `fields` path references them). |
