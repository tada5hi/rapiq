...
Only the `filters` parameter uses the mongo dialect; fields, relations, pagination and sorts accept the same input as the [simple parser](/packages/parser-simple), and the whole thing returns the same [`Query`](/guide/query-ast).
