...
- Filters that traverse a `OneToMany` / `ManyToMany` relation are rendered as a correlated `EXISTS` over the root entity (alias `<alias>_exists`, correlated on every primary key column) instead of joins on the builder, so a record counts once however many related rows match, as in `@rapiq/adapter-memory` and a record read. The joins these filters need run inside the subquery with the same `relations` options, and the `onJoin` hook receives the subquery's builder, so a condition it adds applies there; a builder that shows soft-deleted rows (`withDeleted()`) shows them there too. A join already on your builder is not reused inside the subquery: give a relation its condition through `onJoin`, not through a join of your own under the adapter's alias.
- Aggregates over a to-many join that is on the builder already would count join rows, so a `OneToMany` / `ManyToMany` join you added yourself, or one an `onJoin` hook adds for a to-one filter join, refuses the query (`aggregates:fan-out`).
