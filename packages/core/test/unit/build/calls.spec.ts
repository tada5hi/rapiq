...
describe('src/build/parameter/{groups,aggregates}/*.ts', () => {
    it('should resolve primitives and bare columns against the built-in table', () => {
        ...
        expect(groups.value.map((item) => item.key)).toEqual(['createdAt', 'status']);
    ...
    it('should carry a named function unresolved, in its wire form', () => {
        ...
        expect(aggregates.value[0]!.key).toBe('totalAmount');
    ...
    it('should key aggregates per column', () => {
        ...
        expect(aggregates.value.map((item) => item.key)).toEqual(['count', 'sumAmount', 'sumFee']);
    ...
    it('should key a bucket by its column, so buckets of two columns do not collide', () => {
        const groups = defineGroups([
            { name: 'bucket', params: ['createdAt', 'day'] },
            { name: 'bucket', params: ['updatedAt', 'day'] },
        ]);

        expect(groups.value.map((item) => item.key)).toEqual(['createdAt', 'updatedAt']);
    });

    it('should refuse a column grouped twice, whatever the spelling', () => {
        for (const input of [
            [{ name: 'bucket', params: ['createdAt', 'day'] }, { name: 'bucket', params: ['createdAt', 'hour'] }],
            ['createdAt', { name: 'bucket', params: ['createdAt', 'day'] }],
        ]) {
            const error = captureError<BuildError>(() => defineGroups(input));

            expect(error.code).toBe(ErrorCode.KEY_AMBIGUOUS);
            expect(error.message).toBe(ErrorMessage.groupColumnDuplicate('createdAt'));
        }
    });

    it('should not compare an unresolved group by column', () => {
        const groups = defineGroups([
            { name: 'period', params: ['day'] },
            { name: 'bucket', params: ['createdAt', 'hour'] },
        ]);

        expect(groups.value.map((item) => item.key)).toEqual(['period', 'createdAt']);
    });
    ...
    it('should refuse a key requested twice within a parameter', () => {
        ...
        expect(groups.message).toBe(ErrorMessage.outputKeyDuplicate('period'));
...
describe('src/build/module.ts', () => {
    it('should build a grouped query', () => {
        ...
        expect(query.groups.value.map((item) => item.key)).toEqual(['createdAt', 'status']);
