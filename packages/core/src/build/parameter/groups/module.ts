...
import { Group, Groups } from '../../../parameter';
...
export function defineGroups(input: GroupsBuildInput<ObjectLiteral>) : IGroups {
    if (isParameterNode<IGroups>(input)) {
        return input;
    }
    ...
    const output : IGroup[] = input.map((element) => (isParameterNode<IGroup>(element) ?
        element :
        new Group(buildCallOptions(Parameter.GROUPS, element))));
