...
const pgCases : [Unit, TemporalKind, string][] = [
    ...
    ['hour', 'date', `to_char(date_trunc('hour', "t"."createdAt"::timestamp), ${PG_FORMAT})`],
    ['day', 'date', `to_char(date_trunc('day', "t"."createdAt"::timestamp), ${PG_FORMAT})`],
    ['month', 'date', `to_char(date_trunc('month', "t"."createdAt"::timestamp), ${PG_FORMAT})`],
];

const mysqlCases : [Unit, string][] = [
    ['hour', 'date_format(`t`.`createdAt`, \'%Y-%m-%dT%H:00:00.000Z\')'],
    ['day', 'date_format(`t`.`createdAt`, \'%Y-%m-%dT00:00:00.000Z\')'],
    ['month', 'date_format(`t`.`createdAt`, \'%Y-%m-01T00:00:00.000Z\')'],
];
