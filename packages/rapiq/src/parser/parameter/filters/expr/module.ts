...
import { escapeRegExp, isPathAllowed, isPropertyNameValid } from '../../../../utils';
import { DEFAULT_ID } from '../../../../constants';

/**
 * @see https://www.jsonapi.net/usage/reading/filtering.html
 */
export class ExprFiltersParser extends BaseFiltersParser<
FiltersParseOptions
> {
    ...
    private parseNotExpression(options: FiltersParseOptions = {}, negation: boolean = false): Condition {
    ...
    private parseLogicalExpression(options: FiltersParseOptions = {}, negation: boolean = false): Condition {
    ...
    private parseComparisonExpression(options: FiltersParseOptions = {}, negation: boolean = false): Condition {
    ...
    private parseMatchExpression(options: FiltersParseOptions = {}, negation: boolean = false): Condition {
        const op = this.consume().type;
        this.consume(FilterTokenType.LPAREN);
    ...
    private parseExpressionFieldChain(options: FilterExpressionParseOptions = {}): string {
        ...
        if (options.schema) {
            ...
            let schema = this.resolveSchema(options.schema);
