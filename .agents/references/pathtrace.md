...
| `getPathValue` (`path-value/get.ts`) | `packages/adapter-memory/src/helpers/value.ts` (`resolveProperty`/`resolvePath`) | Deliberately NOT interchangeable: memory resolves **own properties only** (pathtrace uses `in Object(x)`, walking the prototype chain, guarded by `isUnsafeKey`), null-unifies `undefined`→`null`, and resolves arrays-on-path to absent because array fan-out (same-element join-row binding) is owned by `packages/adapter-memory/src/parameter/filters/binding.ts` (plan 014 contract). Memory paths never use `[index]` brackets. |
