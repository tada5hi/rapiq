...
| `EntityMetadata.findRelationWithPropertyPath()` / `findColumnWithPropertyPath()` (`src/metadata/EntityMetadata.ts`, verified on 1.1.0) | `RelationsAdapter.isRelationPath()` + `findColumnByPropertyPath()` (`packages/typeorm/src/adapter/{relations,filters}.ts`) | Embedded column paths (`@Column(() => Profile)` → `profile.firstName`) are dotted but **not** relations: `findRelationWithPropertyPath('profile')` returns `undefined` while `findColumnWithPropertyPath('profile.firstName')` resolves the full remaining path to the embedded column. `isRelationPath` walks relation metadata per segment so `@rapiq/sql`'s `parseField` keeps embedded remainders on the parent alias instead of deriving a join (#779). |
| `DefaultNamingStrategy.columnName()` (`src/naming-strategy/DefaultNamingStrategy.ts`, verified on 1.1.0) | asserted via metadata in `packages/typeorm/test/unit/embedded.spec.ts` | Embedded column database names camelCase the prefixed path with only the first letter after each boundary uppercased: `profile` + `firstName` → `profileFirstname` (not `profileFirstName`). Tests read `findColumnWithPropertyPath(...).databaseName` instead of hardcoding. |
| `SelectQueryBuilder.select()` / `orderBy()` (`src/query-builder/SelectQueryBuilder.ts`, verified on 1.1.0) | `FieldsAdapter` / `SortAdapter` (`packages/typeorm/src/adapter/{fields,sort}.ts`) | Both resolve `alias.propertyPath` keys through entity metadata **including embedded paths** (`user.profile.firstName` → `"user"."profileFirstname"`), so the typeorm fields/sort adapters pass property paths unescaped; only raw WHERE fragments need the database-name resolution (`resolveFieldName`). |
