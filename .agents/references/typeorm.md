...
| TypeORM | rapiq | Notes |
|---------|-------|-------|
| `SelectQueryBuilder` (`src/query-builder/SelectQueryBuilder.ts`) | `TypeormAdapter` (`packages/typeorm/src/adapter/module.ts`) | Builder bound at construction (`{ target: qb }`); `execute(query)` walks the query and applies the state in one call |
