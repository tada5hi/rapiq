...
| TypeORM | rapiq | Notes |
|---------|-------|-------|
| `SelectQueryBuilder` (`src/query-builder/SelectQueryBuilder.ts`) | `TypeormAdapter<TARGET extends SelectQueryBuilder>` (`packages/typeorm/src/adapter/module.ts`) | Adapter mutates the builder via `execute(query, qb)` — walks the query and applies the state in one call |
