...
| `src/query/parameter/filters/module.ts` `transformParsedFilters` | v1 `FiltersParseOutput` (FLAT array of `{key, path, operator, value}`) → `{statement, binding}` list | @rapiq/adapter-sql `FiltersVisitor` + `FiltersBaseAdapter` |
...
| `src/query/parameter/relations/module.ts` | `leftJoinAndSelect` per relation + `onJoin(key, alias, qb)` hook; nested keys via `parts.slice(-2)` | @rapiq/adapter-typeorm `RelationsAdapter` (left join by default, configurable `joinType`, `onJoin` supported) |
| `src/query/parameter/fields/module.ts` + `src/query/utils/alias.ts` | `query.select(fields.map(...))` with alias resolved per field via `getAliasForPath(relationsOutput, field.path)` | @rapiq/adapter-typeorm `FieldsAdapter` + shared `RelationsAdapter` (same cross-parameter alias idea) |
...
## Behavioral facts @rapiq/adapter-typeorm must reckon with (migration contract)
...
4. **Join type**: relations are applied with `leftJoinAndSelect`; @rapiq/adapter-typeorm now preserves this as the default and exposes `relations.joinType: 'inner'` as an opt-in.
5. **`onJoin` hook**: consumers (8+ hub repos, all authup repos) rely on it to `addGroupBy(`${alias}.id`)` because their root queries use `groupBy`. @rapiq/adapter-typeorm exposes the equivalent `relations.onJoin(path, alias, queryBuilder)` hook.
