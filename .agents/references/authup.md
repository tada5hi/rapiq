...
| `apps/server-core/src/app/modules/database/repositories/*/repository.ts` (21 repos; `applyQuery(qb, query, { defaultAlias, fields: { default, allowed }, filters: { allowed }, pagination: { maxLimit: 50 }, relations: { allowed, onJoin: addGroupBy }, sort: { allowed } })`) | typeorm-extension `applyQuery` (wraps v1 parse) | `defineSchema` + `SchemaRegistry` + SimpleParser + @rapiq/adapter-typeorm adapter |
...
- **`relations.onJoin` is used solely to `addGroupBy(`${key}.id`)`** because repos use `qb.groupBy(...)` — v2 @rapiq/adapter-typeorm has no join hook yet (gap, plan 004/010).
