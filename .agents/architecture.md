...
- A site that CANNOT record (a scope built outside a parse; the expression dialect, whose `resolutionThrowOnFailure` forces throwing key resolution because an expression cannot be partially reinterpreted) attaches its issue to the error it throws, and the catching driver merges that trace rather than synthesizing one from the parameter name. A caught abort becomes the raised failure's `cause`: its stack survives without it being mistaken for a description of the whole request.
