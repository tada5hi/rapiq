...
- **Numbers.** `count` and `sum` are normalized with `Number` (one JSON type across pg strings, mysql DECIMAL strings and sqlite numbers, identical to memory); precision beyond 2^53 or on exact decimals is the documented price, the raw driver rows are the escape hatch. A sum over no values is `null`.
