...
- **The raised error is general.** `ParseError` with `ErrorCode.INPUT_REJECTED`, never the first violation's own class or code: a request can violate several policies at once, and an error naming one of them describes a SUBSET, which a consumer branching on the class would then act on. The parameter classes (`PARAMETER_ERROR_CLASSES`) stay what a single violation throws where no trace is collecting. Nothing is ever rebuilt through a consumer's class either, so a custom constructor is never called with options it did not agree to.
