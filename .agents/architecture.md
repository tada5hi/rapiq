...
- **Fan-out.** A join of a to-many relation multiplies `count`/`sum`. typeorm refuses aggregates when ANY `joinAttributes` entry is to-many, caller joins included (`aggregates:fan-out`); standalone adapter-sql has no cardinality and only documents it (the caller owns the join).
- **TypeORM ordering is load-bearing.** Refuse a builder that already has `groupBys` (`groups:builder`, before `clear()` so the builder stays untouched); join only the relations a filter traverses (includes are never joined, in adapter-sql either) and never select them; run the joins before rebuilding the select list and the `GROUP BY` (so an `onJoin` hook's `addGroupBy` is dropped); paginate with `limit`/`offset`, never `take`/`skip`.
