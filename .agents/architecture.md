...
- **`BaseError` extends `@ebec/core`'s**, the house error substrate: class name, stack capture, `code`, `cause` and the `@instanceof` brand chain come from there, and `toJSON` is `super.toJSON()` plus `issues`. So `isBaseError` / `isParseError` (and ebec's own guard) recognize a rapiq error that crossed a boundary as JSON, which `instanceof` cannot. Consequence measured and accepted: the enumerable shape gains ebec's unset `cause`/`errors` fields — cosmetic, since neither varies with input and the trace stays non-enumerable.
