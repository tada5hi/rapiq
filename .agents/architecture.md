...
class Query implements IQuery {
    ...
    readonly aggregates: IAggregates;// Aggregate, same members; key = [name, ...params].join('_')
}
```
...
**Aggregation (settled 2026-09-22, issue #938)**: two optional parameters, `groups` (wire `group`) and `aggregates` (wire `aggregate`), turn a collection read into grouped rows (`GET /events?group=bucket(createdAt,day),scope,name&aggregate=count` answers `{ bucket_createdAt_day, scope, name, count }`). User guide: `packages/docs/guide/grouping.md`; plan: local `.agents/plans/035-aggregation.md`. Settled, do not re-litigate:
...
- **Node shape and output keys.** `name` + `params` are the wire form (re-encoding reproduces `period(day)`), `lowering` the resolved `{ fn, field, args }` (undefined for a client-built named call: transport only, adapters refuse `groups:unresolved`), `key` the purely syntactic output key, `[name, ...params].join('_')` for groups and aggregates alike (revised 2026-09-23: a group key was its bare name, so buckets of two columns collided; `bucket(createdAt,day)` is now `bucket_createdAt_day`, `period(day)` is `period_day`). No `as` syntax. A collision means the same key and nothing else: a key seen twice, within or across the two parameters, is `KEY_AMBIGUOUS` (parse, build, merge, and every grouped entry point). A zero-argument group built without a schema (`defineGroups(['period'])`) is read as a bare column, the only schemaless reading; its wire form is the same, so a zero-argument named function built client-side is only resolved by the server's parse.
