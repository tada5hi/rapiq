...
- **Error identity is `@ebec/core`'s brand chain** (`markInstanceof` / `matchesInstanceof`, one non-enumerable `@instanceof` key holding the ancestor chain), and `BaseError.toJSON` serializes it beside `name`/`message`/`code`/`issues`, so `isBaseError` / `isParseError` also recognize an error that crossed a boundary as JSON. rapiq keeps its own `BaseError`, interfaces and wire shape: only the brand utilities are borrowed.
