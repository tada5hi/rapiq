...
**Aggregation (settled 2026-09-22, issue #938)**: two optional parameters, `groups` (wire `group`) and `aggregates` (wire `aggregate`), turn a collection read into grouped rows (`GET /events?group=bucket(createdAt,day),scope,name&aggregate=count` answers `{ bucket, scope, name, count }`). User guide: `packages/docs/guide/grouping.md`; plan: local `.agents/plans/035-aggregation.md`. Settled, do not re-litigate:
