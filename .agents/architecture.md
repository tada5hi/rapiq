...
Issue traces are bounded by flattened leaves while preserving blemish group shape. Parser-owned structural aborts are priority tail entries, recursive sites carry absolute paths, and filter/sort index policies consult only issues from their own parameter. `received` remains live diagnostic data but is redacted by `BaseError.toJSON()`.
...
- Relation pruning and a parameter's index policy are skipped only after that same parameter's prior failure, so independent `filters` and `sorts` index violations remain visible.
