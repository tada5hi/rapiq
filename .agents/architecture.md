...
- **Grouped mode is driver-owned.** Fields are refused (`FEATURE_UNSUPPORTED`, `fields.default` not applied); relations are parsed (they gate filter traversal) and never hydrated, and an include no filter traverses is refused on the relations parameter (`FEATURE_UNSUPPORTED`, `relations:grouped`; revised 2026-09-23: it was silently dead input). "Traverses" is read off the final filter tree after relation pruning, client filters and `filters.default` alike (`elemMatch` interiors prefixed by their leaf), and a prefix of a traversed path counts, so a schemaless parse is judged the same way (the relation ledger is empty there and does not say which parameter traversed); sorts are parsed against a synthetic unnamed schema allowing only output keys with an empty relations context, so the real schema's `sorts.default`/`validate`/`indexed` do not apply (relation pruning and the sorts index policy skip grouped sorts); `resolveGroupedSorts` appends every group key the explicit sorts do not name, ASC in declared order, as tie-breakers (revised 2026-09-23: the group keys identify a row, so the order is total and LIMIT/OFFSET paging over ties is stable on pg/mysql), which without explicit sorts is every group key ASC; `pagination.maxLimit` caps group rows.
...
- **Numbers.** `count` and `sum` are normalized with `Number` (one JSON type across pg strings, mysql DECIMAL strings and sqlite numbers, identical to memory); precision beyond 2^53 or on exact decimals is the documented price, the raw driver rows are the escape hatch. A sum over no values is `null`. Memory's `sum` reads each value with the same `Number` rule (revised 2026-09-23): finite numbers, bigints and non-blank numeric strings add, since a driver hydrates a decimal as a string and a pg bigint as a string or bigint; anything else is skipped. A `sum` over a non-numeric column is refused typed by the optional `IFiltersAdapter.isNumeric` (base default `true`; typeorm reads metadata: integer, decimal, float and money types or `Number`, never boolean because pg has no `sum(boolean)`) as `aggregates:sum-type`, the `groups:bucket-type` pattern.
